...
namespace Quantum.Kata.Measurements {
    ...
    operation TwoBitstringsMeasurement (qs : Qubit[], bits1 : Bool[], bits2 : Bool[]) : Int {
		//要求You can use exactly one measurement.
        let N = Length(qs);
		mutable j = 0;
		
		repeat {
			set j += 1;
		} until (bits1[j] != bits2[j]);
		mutable b = false;
		if(M(qs[j]) == One) {
			set b = true;
		}
    ...
    operation TwoQubitState (qs : Qubit[]) : Int {
        let res1 = Measure([PauliX],[qs[0]]);
		let res2 = Measure([PauliX],[qs[1]]);
        mutable retValue = 0;
