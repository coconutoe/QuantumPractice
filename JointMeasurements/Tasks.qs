...
namespace Quantum.Kata.JointMeasurements {
    ...
    operation ParityMeasurement (qs : Qubit[]) : Int {
        let res = Measure([PauliZ, PauliZ],[qs[0], qs[1]]);
		let ret = res == Zero ? 0 | 1;
    ...
    operation GHZOrGHZWithX (qs : Qubit[]) : Int {
        let res = Measure([PauliI, PauliZ, PauliI, PauliZ],[qs[0], qs[1], qs[2], qs[3]]);
		let ret = res == Zero ? 0 | 1;
        return ret;
    }
    ...
    operation GHZOrWState (qs : Qubit[]) : Int {
        let N = Length(qs);
		mutable pauliArr = [PauliZ];
		
		for(i in 1..N-1) {
			set pauliArr = pauliArr + [PauliZ];
		}
		let res = Measure(pauliArr,qs);
		let ret = res == Zero ? 0 | 1;
        return ret;
    }
    ...
    operation DifferentBasis (qs : Qubit[]) : Int {
        let res = Measure([PauliX, PauliX],qs);
		let ret = res == Zero ? 0 | 1;
        return ret;
    }
