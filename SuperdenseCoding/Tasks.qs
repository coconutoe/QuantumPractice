...
namespace Quantum.Kata.SuperdenseCoding {
    ...
    operation EncodeMessageInQubit (qAlice : Qubit, message : ProtocolMessage) : Unit {
        ...
        if (not message::Bit1 and message::Bit2) {
			X(qAlice);
        } elif (message::Bit1 and not message::Bit2) {
			Z(qAlice);
        } elif (message::Bit1 and message::Bit2) {
			Y(qAlice);
			//Exp([PauliI], PI()/2.0, [qAlice]);
        }
        // ...
    }
    ...
    operation DecodeMessageFromQubits (qAlice : Qubit, qBob : Qubit) : ProtocolMessage {
		...
		if(res1 == Zero and res2 == Zero) {
			//message::Bit1 = false;
			return ProtocolMessage(false,false);
		} elif(res1 == Zero and res2 == One) {
			X(qBob);
			return ProtocolMessage(false, true);
		} elif(res1 == One and res2 == Zero) {
			X(qAlice);
			return ProtocolMessage(true, false);
		} else {
			X(qAlice);
			X(qBob);
			return ProtocolMessage(true, true);
		}
